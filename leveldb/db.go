...
type DB struct {
	...
	compCntLvl4                  uint32
	dbPath                       string
	totalElapsedPerEpoch         int64
	CountGetDuringCompaction     int32
	getCountDuringCompaction     int64
	totalElapsedDuringCompaction int64

	tickerForCompactionDelay             *time.Ticker
	tickerForParallelCompactionDisable   *time.Ticker
	tickerForRODisable                   *time.Ticker
	getCountForCompactionDelay           int64
	getCountForParallelCompactionDisable int64
}
...
func openDB(s *session) (*DB, error) {
	...
	db := &DB{
		...
		tickerForCompactionDelay:           time.NewTicker(time.Second * 1),
		tickerForParallelCompactionDisable: time.NewTicker(time.Second * 1),
		tickerForRODisable:                 time.NewTicker(time.Second * 1),
	}

	dbPath := reflect.ValueOf(db.s.stor.Storage).Elem().Field(0)
	var strippedDbPath string
	if dbPath.IsValid() {
		strDbPath := dbPath.String()
		strippedDbPath = strings.Replace(strDbPath, "/mnt/ssd/sminkang_test/kend/data/klay", "", -1)
	}
	db.dbPath = strippedDbPath
...
func (db *DB) Get(key []byte, ro *opt.ReadOptions) (value []byte, err error) {
	...
	// KIR
	atomic.AddInt64(&db.getCnt, 1)
	atomic.AddInt64(&db.getCountForCompactionDelay, 1)           // for delay
	atomic.AddInt64(&db.getCountForParallelCompactionDisable, 1) // for MLCC
	flag := atomic.LoadInt32(&db.CountGetDuringCompaction)
	if flag == 1 {
		atomic.AddInt64(&db.getCountDuringCompaction, 1)
	}
	...
	defer func() {
		...
		atomic.AddInt64(&db.totalElapsedPerEpoch, elapsed)
		if flag == 1 {
			atomic.AddInt64(&db.totalElapsedDuringCompaction, elapsed)
		}
	}()
