...
type version struct {
	...
	// KIR
	// cLevels contain levels having cScore equal to or smaller than cLevel
	// and eqaul to or larger than 1.
	// The cScore of cLevels[i] is cScores[i]
	cLevels []int
	cScores []float64
...
// 		} else {
// 			score = float64(size) / (float64(v.s.o.GetCompactionTotalSize(level)) * v.s.o.GetBeta())
// 		}
...
func (v *version) computeCompaction() {
	...
	// KIR
	// Gather scores of each level in order to select compaction candidates
	scores := make([]float64, len(v.levels))
	...
	v.cLevels = make([]int, len(v.levels))
	v.cScores = make([]float64, len(v.levels))

	for level := range v.levels {
		if level != bestLevel && 1 <= scores[level] && scores[level] <= bestScore {
			v.cLevels[level] = level
			v.cScores[level] = scores[level]
		}
	}
}
